...
| `diff`  | `ks`, `hr`, `images`, `all` | Path-keyed diff against `--path-orig` via [dyff](https://github.com/homeport/dyff). K8s-aware: list entries match by identifier (container name, env-var name), so a reorder shows as `⇆ order changed` instead of phantom value churn. `-o` styles: `human` (default), `github`, `brief`, `gitlab`, `gitea`, a plain unified `diff`, and a self-contained `html` page (left navigation tree, per-resource side-by-side ⇄ unified views, YAML syntax highlighting, light/dark theme). |
